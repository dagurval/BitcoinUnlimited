...
public:
    ...
    posix_error(const std::string& operation) :
        subprocess_error(operation + ": " + std::string(strerror(errno)))
    {
    }
...
#define THROW_IF_RETERROR(cond) { \
    int errno_ = cond; \
    if (errno_ != 0) { throw posix_error(#cond, errno_); }\
}
#define THROW_CHECK_ERRNO(cond) { \
    if ((cond) != 0) { throw posix_error(#cond); } \
}
#endif
...
SubProcess::SubProcess(
        ...
        getline_callb stderr_callb_) :
            path(path_),
            args(args_),
            stdout_callb(stdout_callb_),
            stderr_callb(stderr_callb_),
            pid(-1),
            is_running(false),
            run_started(false)
{
...
SubProcess::~SubProcess() {
    ...
    try {
        // Try to recover
        Terminate();
        while (run_started) { std::this_thread::yield(); }
    }
    catch (...) {
...
void SubProcess::Run() {
    ...
    pid_t child_pid;
    THROW_IF_RETERROR(posix_spawnp(&child_pid,
                path.c_str(), &action, nullptr, &cmdargs[0], nullptr));
    pid.store(child_pid);
    ...
    std::vector<pollfd> plist = {
        {cout_pipe[PARENT],POLLIN, 0},
        {cerr_pipe[PARENT],POLLIN, 0}};
    ...
    if (waitpid(pid, &status, 0) == -1) {
        throw std::runtime_error("SubProcess waitpid failed");
    }
